...
public class Puzzle08A extends Puzzle {
        ...
        for (String inputLine : inputLines) {
            ...
            for (String AnswerDigit : answerDigits) {
                ...
                for (char letter : AnswerDigit.toCharArray()) {
                    digit.segments.add(charToSegment.get(letter));
                }

                if (digit.segments.equals(Digit.ONE))
                    cnt++;
                if (digit.segments.equals(Digit.FOUR))
                    cnt++;
                if (digit.segments.equals(Digit.SEVEN))
                    cnt++;
                if (digit.segments.equals(Digit.EIGHT))
                    cnt++;
            }
        }
