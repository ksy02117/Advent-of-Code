...
public class Puzzle18A extends Puzzle {
    ...
    public String solve(String file_path) {
        ...
        Element result = parseElement(new StringCharacterIterator(inputLines.get(0)));

        for (int i = 0; i < inputLines.size(); i++) {
            Element toAdd = parseElement(new StringCharacterIterator(inputLines.get(i)));
            result = addElements(result, toAdd);
        }

        return Integer.toString(result.magnitude());
    }

    // Does not match on test but running with input.txt produces right answer
    @Test
    public void test() {
