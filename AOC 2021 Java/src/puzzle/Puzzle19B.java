...
public class Puzzle19B extends Puzzle {
    ...
    public String solve(String file_path) {
        ...
        List<Coordinate> scannerCoordinates = new ArrayList<>();
        scannerCoordinates.add(new Coordinate(0, 0, 0));
                ...
                for (int i = 0; i < rotatedBeaconCoordinates.size(); i++) {
                    for (int j = 0; j < beacons.size(); j++) {
                        ...
                        if (cnt >= 12) {
                            ...
                            scannerCoordinates.add(new Coordinate(dx, dy, dz));
        ...
        int max = 0;
        for (int i = 0; i < scannerCoordinates.size(); i++) {
            for (int j = i + 1; j < scannerCoordinates.size(); j++) {
                int dx = scannerCoordinates.get(i).x - scannerCoordinates.get(j).x;
                int dy = scannerCoordinates.get(i).y - scannerCoordinates.get(j).y;
                int dz = scannerCoordinates.get(i).z - scannerCoordinates.get(j).z;
